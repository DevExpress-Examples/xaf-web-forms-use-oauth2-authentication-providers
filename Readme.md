...
This example shows how XAF can get a user's email from OAuth2 services and create (or authenticate) a user based on this data ([the AuthenticationStandartWithOAuth.Authenticate method](./CS/AuthenticationOwin.Web/Security/AuthenticationStandartWithOAuth.cs)). Note that third-party API and settings of OAuth2 services (Google, Facebook, and Microsoft) that we use in this example often change and this is out of our control. While we try our best to keep this example up-to-date with these changes, it is always better to refer to the official OAuth2 provider documentation first. Please leave comments or create merge requests to this example if you find any incompatibilies. Known OAuth2 service specificities:
...
- The assembly: Microsoft.Owin.Security.MicrosoftAccount support authenticating to both Microsoft user accounts, and Azure AD (School/Orgnizational) user accounts.  For this demo project to successfully authenticate an Azure AD user account, ensure to configure the Azure AD registered application as multi-tenanted = yes. (Manifest entry: "availableToOtherTenants": true) - added by [nrpieper](https://github.com/nrpieper):
